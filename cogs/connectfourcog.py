        ...
        while True:
                        ...
                        if player_move == "e":
                            ...
                            for move in valid:
                                eval_str += (
                                    move
                                    + ": "
                                    + str(-compute_move("X", move, 0, 3, board))
                                    + "\n"
                                )
                                remove_move(move, board)

                            await ctx.send(
                                f"These only go to depth 3. They are not a perfect eval for sure.\n{eval_str}"
                            )

                        # Gives all the options for training mode
                        await ctx.send(
                            "Please enter a valid move, q to quit, b to take back your move, or e to see evaluations"
                        )
                        msg = await self.bot.wait_for("message", check=check)
            ...
            # If it's the computer's move
                ...
                if training:
                    ...
                    for move in valid:
                        player_score = max(
                            player_score, -1 * compute_move("X", move, 0, 3, board)
                        )
                        remove_move(move, board)
                    ...
                    if player_score > 0:
                        await ctx.send(
                            f"You have an advantage of {player_score} heuristic points!"
                        )
                    elif player_score < 0:
                        await ctx.send(
                            f"The computer has an advantage of {-player_score} heuristic points!"
                        )
                    else:
                        await ctx.send(
                            f"You and the computer are even on heuristic points!"
                        )
