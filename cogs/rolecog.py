...
class RoleCog(commands.Cog):
    ...
    async def add_remove_role(self, payload, add):
        ...
        valid_emoji_lists = map(
            lambda listener: listener.keys(), self._listeners.values()
        )
        valid_emojis = [item for sublist in valid_emoji_lists for item in sublist]

        if str(payload.emoji) in valid_emojis:
            corresponding_role = guild.get_role(
                self._listeners[str(message.id)][str(payload.emoji)]
            )
            ...
            add
        ):  # In the case that someone reacted with an emoji that doesn't correspond to a role
